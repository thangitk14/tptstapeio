...
public class TPTStape {
    
    public typealias EventResult = Result<TPTStape.EventResponse, SendError>
    ...
    private enum State {
        ...
        func handleStart(_ stape: TPTStape, configuration: Configuration) -> State {
            stape.apiCLient.config = configuration
            return .running(configuration)
        }
        
        func handleFBHookInstall(stape: TPTStape) -> State {
            STFirebaseHook.installLogEventHook { name, parameters in
                if let name = name as? String,
                   let payload = parameters as? [String: AnyHashable] {
                    TPTStape.send(event: TPTStape.Event(name: name, payload: payload)) { result in
        ...
        func handleEvent(_ stape: TPTStape, event: Event, completion: Completion? = nil) -> State {
    ...
    private static var shared: TPTStape = { return TPTStape(apiCLient: APIClient()) }()
